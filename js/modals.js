...
function openUpiApp(appName) {
  ...
  const note = encodeURIComponent(`Order Payment to ${upiId}`);

  // Detect iOS vs Android
  const userAgent = navigator.userAgent || navigator.vendor || window.opera;
  const isIOS = /iPad|iPhone|iPod/.test(userAgent) && !window.MSStream;

  // Standard Universal UPI URI (Guarantees payment goes to merchant specified UPI ID)
  const standardUpiUri = `upi://pay?pa=${encodeURIComponent(upiId)}&pn=${payeeName}&am=${amount}&cu=INR&tn=${note}`;

  let targetUri = standardUpiUri;

  if (isIOS) {
    // iOS Specific Scheme Handlers targeting specified UPI ID
    if (appName === 'gpay') {
      targetUri = `gpay://upi/pay?pa=${encodeURIComponent(upiId)}&pn=${payeeName}&am=${amount}&cu=INR&tn=${note}`;
    } else if (appName === 'phonepe') {
      targetUri = `phonepe://pay?pa=${encodeURIComponent(upiId)}&pn=${payeeName}&am=${amount}&cu=INR&tn=${note}`;
    } else if (appName === 'paytm') {
      targetUri = `paytmmp://pay?pa=${encodeURIComponent(upiId)}&pn=${payeeName}&am=${amount}&cu=INR&tn=${note}`;
    } else {
      targetUri = standardUpiUri;
    }
  } else {
    // Android Specific App Scheme Handlers targeting specified UPI ID
    if (appName === 'gpay') {
      targetUri = `tez://upi/pay?pa=${encodeURIComponent(upiId)}&pn=${payeeName}&am=${amount}&cu=INR&tn=${note}`;
    } else if (appName === 'phonepe') {
      targetUri = `phonepe://pay?pa=${encodeURIComponent(upiId)}&pn=${payeeName}&am=${amount}&cu=INR&tn=${note}`;
    } else if (appName === 'paytm') {
      targetUri = `paytmmp://pay?pa=${encodeURIComponent(upiId)}&pn=${payeeName}&am=${amount}&cu=INR&tn=${note}`;
  ...
  showToast(`Opening ${appDisplayName} (Pay to ${upiId})... 📲`, 'info');
      ...
      ${paymentMethod.includes('UPI') ? `
        <div style="background: var(--bg-white); border: 2px solid var(--accent); border-radius: var(--radius-sm); padding: 16px; margin-bottom: 16px; text-align: center;">
          ...
          <div style="display: flex; gap: 6px; justify-content: center; margin-bottom: 12px; flex-wrap: wrap;">
            <button type="button" class="btn" onclick="openUpiApp('gpay')" style="flex: 1; background: #FFFFFF; color: #3C4043; border: 1px solid #DADCE0; font-size: 0.78rem; font-weight: 700; padding: 8px 6px; border-radius: 6px;"><span style="color: #4285F4; font-weight: 800;">G</span>Pay</button>
            <button type="button" class="btn" onclick="openUpiApp('phonepe')" style="flex: 1; background: #5F259F; color: white; border: none; font-size: 0.78rem; font-weight: 700; padding: 8px 6px; border-radius: 6px;"><span style="background: white; color: #5F259F; border-radius: 50%; padding: 0 4px; font-size: 0.68rem;">पे</span> PhonePe</button>
            <button type="button" class="btn" onclick="openUpiApp('paytm')" style="flex: 1; background: #002E6E; color: white; border: none; font-size: 0.78rem; font-weight: 700; padding: 8px 6px; border-radius: 6px;"><span style="color: #00BAF2; font-weight: 800;">Paytm</span></button>
